...
sudo /usr/bin/time --verbose perf stat -a --per-socket -e dTLB-load-misses,dTLB-loads,dTLB-store-misses,dTLB-stores,cache-misses,cache-references,bus-cycles -- taskset -c 0,2,4,6,8,10,12,14 bash /local/gapbs/bc -f gapbs/benchmark/graphs/kron.sg -n 20
cat /proc/vmstat | grep numa_pages_migrated
cat /proc/vmstat | grep pgpromote_success
cat /proc/vmstat | grep nr_active_file
