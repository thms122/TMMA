...
#!/bin/bash
source /local/llama/venv/bin/activate
...
BENCH_NAMES=("llama")
BENCH_CMDS=(
/local/llama.cpp/Meta-Llama-3-70B-Instruct-Q4_K_M.gguf   -t 8   -p 64   -n 128
)
# Parameter values
...
for (( id = start_index; id < TOTAL; id++ )); do
    ...
    -- taskset -c 0,1,2,3,4,5,6,7 /local/llama.cpp/build/bin/llama-bench   -m /local/llama.cpp/Meta-Llama-3-70B-Instruct-Q4_K_M.gguf   -t 8   -p 64   -n 128 | sudo tee -a "$logfile"
