...
EXPORT_SYMBOL_GPL(node_is_toptier); //M_ Tier Creation
...
EXPORT_SYMBOL_GPL(next_demotion_node); //M_ Tier Creation
...
*/
        ...
        pr_info("M: top_tier_adistance set to %d", top_tier_adistance);

//	End of Tier Creation 
...
struct memory_dev_type *M_get_default_dram_memtype(void) {
        return default_dram_type;
}
EXPORT_SYMBOL_GPL(M_get_default_dram_memtype);

void M_init_memory_tier(int node) {
        struct memory_tier *memtier;

        if (node_state(node, N_CPU)) {
                pr_info("M: NUMA node is NOT zero CPU");
        } else {
                pr_info("M: NUMA node is zero CPU");
        }
...
EXPORT_SYMBOL_GPL(M_init_memory_tier);

void M_clear_memory_tier(int node) {
//      struct memory_tier *memtier;
...
EXPORT_SYMBOL_GPL(M_clear_memory_tier);
// End M_ Tier Creation
